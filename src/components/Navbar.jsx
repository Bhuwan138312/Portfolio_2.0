...
export default function Navbar() {
  ...
  const measureLinks = useCallback(() => {
    ...
    // Clear inline transforms temporarily so we measure base layout positions
    links.forEach(l => {
  ...
  const computeTarget = useCallback(() => {
    ...
    // Add slight padding to pill to comfortably fit the scaled text
  ...
  useEffect(() => {
    const loop = () => {
      ...
      links.forEach(l => {
        ...
        const scale = 1 + ov * 0.12; // Slightly reduced scale for better fit
        
        el.style.transform = `scale(${scale})`;
        el.style.color = ov > 0.05 ? `hsl(145,${Math.round(ov * 42)}%,${Math.round(44 - ov * 8)}%)` : '';
        // Removed font-weight change to prevent layout shifts which cause pill misalignment
      });
